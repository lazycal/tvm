...
std::pair<Array<Layout>, Array<Layout>> BinaryBroadcastLayoutHelper(
  ...
  // (1) infer in_layouts before rewrite
  // (2) if some in_layout is changed by its producer after rewrite, we will rewrite the other
  // layout to make sure it's changed in the same way, in order to keep them broadcastable.
  ...
  // first operate on the original layouts
  ...
  if (!layouts[0].defined() && !layouts[1].defined()) {
    ...
    if (old_in_shapes[defined_idx].size() >= old_in_shapes[undef_idx].size()) {
      // TODO: handle the case when the sublayout contains subcoordinate of factor one but the other
      // tensor has the corresponding dimension size other than one.
  ...
  // otherwise, rewrite the layouts to make them broadcastable again.
  ...
  // Apply a greedy strategy that always transform the small layout
  Layout tgt_layout =
      TryTransformLike(layouts[small_idx], layouts[large_idx], new_in_layouts[large_idx]);
  if (!tgt_layout.defined()) return out_default;
