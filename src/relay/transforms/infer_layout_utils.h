 ...
 * will be expanded. For example, if old = 'NW', ref_old = 'NC', ref_new = 'NC1c', then the result
 * is 'NW1w'. if old = 'W', ref_old = 'NC', ref_new = 'NC1c', then the result is 'NW1w'. When `old`
 * and `ref_old` are isomorphic (same structure, only differ in naming), the transform is guaranteed
 * to succeed, in which case the function is simply renaming the axes of `ref_new` to conform to
 * `old`'s naming.
 * \param old The layout to be transformed.
 * \param ref_old The reference layout
 * before transform.
